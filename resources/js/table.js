...
const APIEndpoint =
  "http://localhost/stock-management/app/api/data-service.php";
...
const updateTable = (data) => {
  ...
  for (
    ...
    for (
      let j = 0;
      j < data[i].length;
      j++ // looping trough <td> in each row, except the last td;
    ) {
      ...
      if (indexOfDescription === j) {
        // if this is the description column then make it hidden;
        td.style.display = "none";
      }
      if (i === data.length - 1) {
        // if this is the last row in the table then add to each td tag a "last" class;
    ...
    if (["products", "orders", "supplierOrders"].indexOf(viewName) !== -1) {
